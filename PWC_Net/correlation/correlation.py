...
class _FunctionCorrelation(torch.autograd.Function):
    ...
    def forward(self, one, two):
        ...
        if one.is_cuda == True:
            ...
            # print(f"🔍 rbot0.shape: {rbot0.shape}")
            # print(f"🔍 rbot1.shape: {rbot1.shape}")
            # print(f"🔍 准备调用CUDA内核...")
            # print(f"🔍 grid: {tuple([ output.shape[3], output.shape[2], output.shape[0] ])}")
            # print(f"🔍 block: {tuple([ 32, 1, 1 ])}")
